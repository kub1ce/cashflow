...
class API:
    ...
    # ── Управление отдельными транзакциями (ФАКТ) ──────────────────────────────
    def add_fact_transaction(self, data: dict) -> dict:
        ...
        try:
            ...
            # Обработка парных транзакций (Копилка)
            cats_to_add = [cat]
            if cat.name == 'Покрытие из копилки':
                paired = self._get_or_create_system_category(db, 'В копилку', 'expense', '#8b5cf6')
                cats_to_add.append(paired)
            elif cat.name == 'В копилку':
                paired = self._get_or_create_system_category(db, 'Покрытие из копилки', 'income', '#0ea5e9')
                cats_to_add.append(paired)
            ...
            for c in cats_to_add:
                f = Fact(
                    ...
                    amount=amount,
                    date=data['date'],
                    comment=None, # Комментарии к отдельным транзакциям (пока null, можно добавить в будущем)
                    external_id=None
                )
                db.add(f)
                db.flush()
    ...
    def update_fact_transaction(self, data: dict) -> dict:
        ...
        try:
            ...
            if cat and cat.name in ('В копилку', 'Покрытие из копилки'):
                ...
                if paired_cat:
                    # Ищем парную транзакцию (совпадает дата, сумма и неделя)
                    ...
                    # Если нашли - обновляем и её тоже
                    if paired_fact:
        ...
        try:
            ...
            if cat and cat.name in ('В копилку', 'Покрытие из копилки'):
                ...
                if paired_cat:
                    ...
                    # Если нашли - удаляем её вместе с основной
                    if paired_fact:
    ...
    def _today_local(self) -> str:
        """Возвращает сегодняшнюю дату в локальном часовом поясе системы"""
        try:
            # Python 3.9+ — берём часовой пояс прямо из системы
            local_tz = datetime.now(timezone.utc).astimezone().tzinfo
            today = datetime.now(tz=local_tz).date()
        except Exception:
            # Fallback — хотя бы UTC лучше чем сломаться
            today = date.today()
